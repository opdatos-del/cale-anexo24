...
BEGIN
    ...
    IF @Archivo IS NULL OR @Hash IS NULL OR LEN(@Hash) <> 64 OR @UsuarioId IS NULL
       ...
       OR ISJSON(@ErroresJson) <> 1 OR @CorrelationId IS NULL
       OR LTRIM(RTRIM(@CorrelationId)) = ''
    ...
    BEGIN TRY
        ...
        IF @CargaId IS NULL OR @CargaId <= 0
        ...
        DECLARE @Detalle VARCHAR(500) = CONCAT('hash=', LEFT(@Hash, 12),
            ';cargaId=', @CargaId, ';total=', @TotalRegistros,
