...
| Administración | Administración de usuarios (CRUD) | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` | QUERY | Sí | No | **READ-ONLY IMPLEMENTADO EN REPOSITORIO** (FASE 2) — `GET /api/v1/administracion/usuarios` y `/{id}` con `USUARIOS_ADMINISTRAR`, JOIN `PerfilApp`, sin `password_hash`, sin DML; escrituras NO IMPLEMENTADAS (FASE 3) |
