...
| Administración | Administración de usuarios Fase 3A | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` + `BitacoraEvento` | COMMAND/QUERY | Sí | Sí | **IMPLEMENTADO EN REPOSITORIO; PENDIENTE DE VALIDACIÓN RUNTIME REMOTA** — `GET` listado/detalle, `POST` y `PUT` protegidos por `USUARIOS_ADMINISTRAR`; create/edit de nombre-correo, BCrypt, auditoría y transacción con `appTransactionManager`. Grants versionados: `UsuarioApp SELECT, INSERT, UPDATE (nombre, correo)`; `PerfilApp SELECT`; `PerfilActividad SELECT`; `Actividad SELECT`; `BitacoraEvento SELECT, INSERT`; despliegue runtime pendiente. Fase 3B (estado/perfil/vigencia/guardrails) y Fase 3C (reset password) pendientes |
