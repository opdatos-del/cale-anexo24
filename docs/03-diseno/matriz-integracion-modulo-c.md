...
| Bitácora | Consulta de eventos propios de aplicación | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | QUERY | Sí | No | IMPLEMENTADO, INTEGRADO EN DEV, VALIDADO LIVE — `GET /api/v1/bitacora` con `BITACORA_CONSULTAR`, rango UTC, filtros exactos, paginación y orden estable; fuente propia, no Módulo C; runtime EXECUTE-only |
| Bitácora | Registro interno de eventos críticos | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | AUDIT/COMMAND | Sí | Sí | IMPLEMENTADO, INTEGRADO EN DEV, VALIDADO LIVE — writer append-only mediante `APP24_C_BITACORA_REGISTRAR` para `LOGIN_OK`, `LOGIN_FALLIDO`, `USUARIO_CREADO`, `USUARIO_ACTUALIZADO` y acciones Fase 3B. Los commands registran negocio + bitácora mediante `appTransactionManager`; 11 EXECUTE específicos, cero grants directos de tablas. 401/403 y acciones restantes pendientes |
...
| Administración | Autenticación de usuario | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` + `PerfilActividad` + `Actividad` | QUERY | Sí | No | CONFIRMADO — `POST /api/v1/auth/login` implementado; JWT HS256 con claims `uid`/`auth`, bcrypt, actor confiable; fuente propia, no Módulo C. FASE 1 (IMPLEMENTADO EN REPOSITORIO): `PerfilApp.estado` efectivo, perfil INACTIVO → 401 genérico, perfil ACTIVO sin permisos autentica |
| Administración | Administración de usuarios Fases 3A/3B | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` + `PerfilActividad` + `Actividad` + `BitacoraEvento` | COMMAND/QUERY | Sí | Sí | **IMPLEMENTADO, INTEGRADO EN DEV, VALIDADO LIVE** — `GET` listado/detalle, `POST`, `PUT` y tres `PATCH` (estado, perfil, vigencia), protegidos por `USUARIOS_ADMINISTRAR`; creación/edición, BCrypt, auditoría y transacciones con `appTransactionManager`. Los `PATCH` usan `SERIALIZABLE`; commands atómicos usan guardrail y locking; impiden auto-inactivación y cambio real de perfil propio, y tras cada mutación preservan al menos un administrador efectivo. JWT emitidos no se revocan. Runtime: `app24_runtime`, 11 EXECUTE específicos, cero grants directos y ownership chain compatible. Fase 3C (reset password) pendiente. Compatibilidad legacy SAT/bloqueo/edición administrativa sin resolver |
| Administración | Administración de perfiles y PerfilActividad | APP DATABASE | `ANEXO24_DEV.app24.PerfilApp` + `PerfilActividad` | COMMAND/QUERY | — | — | PENDIENTE — sin endpoints ni DML administrativo; acceso runtime directo a tablas = 0; futuros SP/commands requieren fase propia |
| Administración | Catálogo de actividades/permisos | APP DATABASE | `ANEXO24_DEV.app24.Actividad` | QUERY | — | — | PENDIENTE — sin endpoint; catálogo seed controlado; acceso runtime directo a tablas = 0 |
