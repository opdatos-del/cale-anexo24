...
| Bitácora | Consulta de eventos propios de aplicación | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | QUERY | Sí | No | DECISIÓN V1 candidata — fuente canónica propia, no Módulo C; `GET /api/v1/bitacora` candidato con `BITACORA_CONSULTAR`, rango/paginación y orden pendientes |
| Bitácora | Registro interno de eventos críticos | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | AUDIT/COMMAND | Sí | Sí | IMPLEMENTADO EN REPOSITORIO — writer interno append-only (`BitacoraEventoRepository`/`RegistrarEventoBitacoraService`/`BitacoraJdbcAdapter`), no endpoint público; permisos mínimos `app24_runtime` pendientes de despliegue y conexiones de eventos/transaccionalidad pendientes por command |
