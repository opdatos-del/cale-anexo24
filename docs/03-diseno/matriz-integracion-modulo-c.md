...
| Bitácora | Registro interno de eventos críticos | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | AUDIT/COMMAND | Sí | Sí | IMPLEMENTADO EN REPOSITORIO; PENDIENTE DE VALIDACIÓN RUNTIME REMOTA — writer append-only para `LOGIN_OK`, `LOGIN_FALLIDO`, `USUARIO_CREADO` y `USUARIO_ACTUALIZADO`. Los commands Fase 3A registran negocio + bitácora mediante `appTransactionManager`; grants versionados `SELECT, INSERT` pendientes de despliegue. 401/403 y acciones restantes pendientes |
...
| Administración | Administración de usuarios Fases 3A/3B | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` + `PerfilActividad` + `Actividad` + `BitacoraEvento` | COMMAND/QUERY | Sí | Sí | **IMPLEMENTADO EN REPOSITORIO; PENDIENTE DE VALIDACIÓN RUNTIME REMOTA** — `GET` listado/detalle, `POST`, `PUT` y tres `PATCH` (estado, perfil, vigencia), protegidos por `USUARIOS_ADMINISTRAR`; creación/edición, BCrypt, auditoría y transacciones con `appTransactionManager`. Los `PATCH` usan `SERIALIZABLE`; impiden auto-inactivación y cambio real de perfil propio, y tras cada mutación preservan al menos un administrador efectivo: usuario ACTIVO, vigencia `null` o `>= LocalDate.now()` de JVM, perfil ACTIVO y ambas actividades `USUARIOS_ADMINISTRAR`/`PERFILES_ADMINISTRAR`. JWT emitidos no se revocan. Grants versionados: `UsuarioApp SELECT, INSERT, UPDATE (nombre, correo, estado, perfil_id, vigencia)`; `PerfilApp SELECT`; `PerfilActividad SELECT`; `Actividad SELECT`; `BitacoraEvento SELECT, INSERT`; `UPDATE (estado, perfil_id, vigencia)` pendiente de despliegue remoto. Fase 3C (reset password) pendiente. Compatibilidad legacy SAT/bloqueo/edición administrativa sin resolver |
| Administración | Administración de perfiles y PerfilActividad | APP DATABASE | `ANEXO24_DEV.app24.PerfilApp` + `PerfilActividad` | COMMAND/QUERY | — | — | PENDIENTE — sin endpoints ni DML administrativo en Fase 3A; grants actuales solo `SELECT` |
| Administración | Catálogo de actividades/permisos | APP DATABASE | `ANEXO24_DEV.app24.Actividad` | QUERY | — | — | PENDIENTE — sin endpoint en Fase 3A; catálogo seed controlado y grant actual `SELECT` |
