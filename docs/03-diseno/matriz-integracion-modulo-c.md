...
| Bitácora | Consulta de eventos propios de aplicación | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | QUERY | Sí | No | DECISIÓN V1 candidata — fuente canónica propia, no Módulo C; `GET /api/v1/bitacora` candidato con `BITACORA_CONSULTAR`, rango/paginación y orden por cerrar en implementación tras ajustar inmutabilidad |
| Bitácora | Registro interno de eventos críticos | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | AUDIT/COMMAND | Sí | Sí | DECISIÓN V1 candidata — writer interno, no endpoint público; DDL actual no garantiza inmutabilidad porque bootstrap concede `db_datawriter`; diseñar actor JWT, privilegios y transaccionalidad antes de implementar |
