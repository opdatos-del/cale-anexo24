...
| Bitácora | Registro interno de eventos críticos | APP DATABASE | `ANEXO24_DEV.app24.BitacoraEvento` | AUDIT/COMMAND | Sí | Sí | IMPLEMENTADO, VALIDADO LIVE — writer append-only mediante `APP24_C_BITACORA_REGISTRAR` para `LOGIN_OK`, `LOGIN_FALLIDO`, `USUARIO_CREADO`, `USUARIO_ACTUALIZADO`, acciones Fase 3B y `USUARIO_PASSWORD_RESTABLECIDA`. Los commands registran negocio + bitácora mediante `appTransactionManager`; 12 EXECUTE específicos, cero grants directos de tablas. 401/403 y acciones de perfiles permanecen pendientes |
...
| Administración | Administración de usuarios Fases 3A/3B/3C | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` + `PerfilActividad` + `Actividad` + `BitacoraEvento` | COMMAND/QUERY | Sí | Sí | **IMPLEMENTADO Y VALIDADO LIVE** — `GET` listado/detalle, `POST` creación/reset password, `PUT` y tres `PATCH` (estado, perfil, vigencia), protegidos por `USUARIOS_ADMINISTRAR`; BCrypt, auditoría y transacciones con `appTransactionManager`. Los `PATCH` usan `SERIALIZABLE`; commands atómicos usan guardrail y locking. Reset reutiliza `PasswordPolicy`, envía sólo hash a SQL y no revoca JWT. Runtime: `app24_runtime`, 12 EXECUTE específicos, cero grants directos y ownership chain compatible. Fases 3A/3B están integradas en DEV; Fase 3C queda pendiente de integración. Compatibilidad legacy SAT/bloqueo/edición administrativa sin resolver |
