...
| Administración | Administración de usuarios (CRUD) | APP DATABASE | `ANEXO24_DEV.app24.UsuarioApp` + `PerfilApp` | COMMAND/QUERY | Sí | Sí | **IMPLEMENTADO EN REPOSITORIO; PENDIENTE DE VALIDACIÓN RUNTIME REMOTA** — FASE 2 read-only y FASE 3A `POST`/`PUT` base con `USUARIOS_ADMINISTRAR`, appJdbcTemplate, password bcrypt y Bitácora transaccional; estado/perfil/vigencia/reset password pendientes |
| Administración | Administración de perfiles | APP DATABASE | `ANEXO24_DEV.app24.PerfilApp` + `PerfilActividad` | COMMAND | — | — | NO IMPLEMENTADO — DECISIÓN V1 — seed `ADMINISTRADOR`/`CONSULTA`; permiso `PERFILES_ADMINISTRAR` sin backend; FASE 1 hace que el perfil INACTIVO bloquee autenticación (CRUD sin implementar) |
