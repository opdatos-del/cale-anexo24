...
/** Pruebas web y de autorización del catálogo de Productos. */
...
class ProductControllerTest {
    ...
    void sinTokenResponde401ConApiError() throws Exception {
        mockMvc.perform(get("/api/v1/catalogos/productos"))
    ...
    void autorizadoResponde200() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/catalogos/productos")
                        .with(user("usuario").authorities(new SimpleGrantedAuthority("PRODUCTOS_CONSULTAR"))))
    ...
    void parametrosInvalidosResponden400() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/catalogos/productos?pagina=0")
                        .with(user("usuario").authorities(new SimpleGrantedAuthority("PRODUCTOS_CONSULTAR"))))
    ...
    void errorDeBaseDeDatosResponde503() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/catalogos/productos?filtro=abc")
                        .with(user("usuario").authorities(new SimpleGrantedAuthority("PRODUCTOS_CONSULTAR"))))
